...
/**
@Todo : use the new payment method field of a property to filter searches 
...
export const searchPlace = functions.https.onRequest((req, res) =>{
	...
	const institution_and_campus = req.query.institution_and_campus;
	const funding_type = req.query.funding_type; 
	const parking_needed = req.query.parking_needed;
	//const special_needs = req.query.special_needs;
	const db = admin.firestore();
	let apartments_ref: any;

	if((room_type != "any") && (funding_type == "self") && parking_needed){
		apartments_ref = db.collection('Apartments')
		.where('rent', '<=', max_price)
		.where('room_type', '==', room_type)
		.where('property.parking', '==', true)

	}else if((room_type != "any") && (funding_type == "self")&& !parking_needed){
		apartments_ref = db.collection('Apartments')
		.where('rent', '<=', max_price)
		.where('room_type', '==', room_type)
	}else if((room_type != "any") && (funding_type != "self")&& !parking_needed){
	...
	const apartments = apartments_ref.get();
